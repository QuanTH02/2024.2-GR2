...
def compute_bleu(config, pred_out, gold_text):
    ...
    # Clean and encode text to handle Unicode issues
    def clean_text(text):
    ...
    pred_out_clean = clean_text(pred_out)
    gold_text_clean = clean_text(gold_text)
    
    with open(output_fn, 'w', encoding='utf-8') as f, open(gold_fn, 'w', encoding='utf-8') as f1:
        if config['task'] == 'summarize':
            predictions = []
            gold_list = []
            predictions.append(str(1) + '\t' + pred_out_clean)
            # gold_list.append(str(1) + '\t' + gold_text_clean + '\n')
            # f.write(str(1) + '\t' + pred_out_clean + '\n')
            gold_text_clean = gold_text_clean.replace('\n', ' ')
            f1.write(str(1) + '\t' + gold_text_clean + '\n')
        elif config['task'] == 'translation' or config['task'] == 'refinement':
            f.write(pred_out_clean + '\n')
            f1.write(gold_text_clean + '\n')
...
def attack(feature, config, victim_model, atk_model, max_length=512, threshold_pred_score=0.3):
    ...
    for top_index in index_important_words:
        ...
        for substitute_ in substitutes:
            ...
            adv_score = torch.sum(adv_scores)
